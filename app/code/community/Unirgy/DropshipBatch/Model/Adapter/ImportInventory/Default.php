    ...
    public function process($rows)
    {
        ...
        if (!empty($_vendorSkus) && $allowVendorSku) {
            ...
            if (Mage::helper('udropship')->isUdmultiAvailable()) {
                ...
                $_tmpSel->columns(array('vp.vendor_sku', 'GROUP_CONCAT(entity_id)'));
                ...
                $_tmpSel->columns(array('vp.value', 'GROUP_CONCAT(entity_id)'));
        ...
        foreach ($rows as $i=>&$r) {
            ...
            if (empty($r['sku']) && empty($r['vendor_sku'])) {
                ...
                && $_pIdsCnt==1
                && !array_key_exists($skuPids[$r[$skuKey]], $this->getVendor()->getTableProducts())
            ) {
                if (!$_newAssocAllowed) {
